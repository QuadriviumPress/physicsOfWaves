...
Consider first the free oscillation of a damped oscillator. This could be, for example, a system of a block attached to a spring, like that shown in [Figure 1.1](#fig-1-1), but with the whole system immersed in a viscous fluid. Then in addition to the restoring force from the spring, the block experiences a frictional force. For small velocities, the frictional force can be taken to have the form 
$$
-m \Gamma v , \tag{2.1} \label{eq-2-1}
$$

where $\Gamma$ is a constant. Notice that because we have extracted the factor of the mass of the block in [2.1](#eq-2-1), $1 / \Gamma$ has the dimensions of time. We can write the equation of motion of the system as 
$$
\frac{d^{2}}{d t^{2}} x(t)+\Gamma \frac{d}{d t} x(t)+\omega_{0}^{2} x(t)=0 , \tag{2.2} \label{eq-2-2}
$$

where $\omega_{0}=\sqrt{K / m}$. This equation is linear and time translation invariant, like the undamped equation of motion. In fact, it is just the form that we analyzed in the previous chapter, in [1.16](#eq-1-16). As before, we allow for the possibility of complex solutions to the same equation, 
$$
...
where $\alpha$ (Greek letter alpha) is a constant. Putting [2.4](#eq-2-4) into [2.2](#eq-2-2), we find 
$$
...
From [2.6](#eq-2-6), we see that there are three regions for $\Gamma$ compared to $\omega_{0}$ that lead to different physics.

### Overdamped Oscillators

If $\Gamma / 2>\omega_{0}$, both solutions for $\alpha$ are real and negative. The solution to [2.2](#eq-2-2) is a sum of decreasing exponentials. Any initial displacement of the system dies away with no oscillation. This is an **overdamped oscillator**.

The general solution in the overdamped case has the form, 
$$
x(t)=z(t)=A_{+} e^{-\Gamma_{+} t}+A_{-} e^{-\Gamma_{-} t}, \tag{2.7} \label{eq-2-7}
$$

where 
$$
\Gamma_{\pm}=\frac{\Gamma}{2} \pm \sqrt{\frac{\Gamma^{2}}{1}-\omega_{0}^{2}} . \tag{2.8} \label{eq-2-8}
$$

:::{figure} ../images/lt-34037-clipboard_ec961342c00f84c1a94c264467e360ea7.png
:label: fig-2-1
:enumerator: 2.1
:alt: Solutions to the equation of motion for an overdamped oscillator.

Solutions to the equation of motion for an overdamped oscillator.
:::
An example is shown in [Figure 2.1](#fig-2-1). The dotted line is $e^{-\Gamma+t}$ for $\Gamma = 1 s^{-1}$ and $\omega_{0}=.4 \mathrm{~s}^{-1}$. The dashed line is $e^{-\Gamma_{-} t}$. The solid line is a linear combination, $e^{-\Gamma_{+} t}-\frac{1}{2} e^{-\Gamma_{-} t}$.

In the overdamped situation, there is really no oscillation. If the mass is initially moving very fast toward the equilibrium position, it can overshoot, as shown in [Figure 2.1](#fig-2-1). However, it then moves exponentially back toward the equilibrium position, without ever crossing the equilibrium value of the displacement a second time. Thus in the free motion of an overdamped oscillator, the equilibrium position is crossed either zero or one times.
...
$$
\omega^{2}=\omega_{0}^{2}-\Gamma^{2} / 4 . \tag{2.9} \label{eq-2-9}
$$
...
Thus $t e^{-\Gamma t / 2}$ is also a solution. You can also check this explicitly, by inserting it back into [2.2](#eq-2-2). This is called the **critically damped** case because it is the boundary between overdamping and underdamping.
...
$$
c e^{-\Gamma t / 2}+d t e^{-\Gamma t / 2} . \tag{2.13} \label{eq-2-13}
$$

This is illustrated in [Figure 2.3](#fig-2-3). The dotted line is $e^{-\Gamma t}$ for $\Gamma=1 \mathrm{~s}^{-1}$. The dashed line is $t e^{-\Gamma t}$. The solid line is a linear combination, $(1-t) e^{-\Gamma t}$.

:::{figure} ../images/lt-34039-clipboard_e0e0cf4395f8711d8845797890d0f673a.png
:label: fig-2-3
:enumerator: 2.3
:alt: Solutions to the equation of motion for a critically damped oscillator.
...
where the force is 
$$
F(t)=F_{0} \cos \omega_{d} t . \tag{2.15} \label{eq-2-15}
$$

The $\omega_{d} / 2 \pi$ is called the driving frequency. Notice that it is not necessarily the same as the natural frequency, $\omega_{0} / 2 \pi$, nor is it the oscillation frequency of the free system, [2.9](#eq-2-9). It is simply the frequency of the external force. It can be tuned completely independently of the other parameters of the system. It would be correct but awkward to refer to $\omega_{d}$ as the driving angular frequency. We will simply call it the driving frequency, ignoring its angular character.

The angular frequencies, $\omega_{d}$ and $\omega_{0}$, appear in the equation of motion, [2.15](#eq-2-15), in completely different ways. You must keep the distinction in mind to understand forced oscillation. The natural angular frequency of the system, $\omega_{0}$, is some combination of the masses and spring constants (or whatever relevant physical quantities determine the free oscillations). The angular frequency, $\omega_{d}$, enters only through the time dependence of the driving force. This is the new aspect of forced oscillation. To exploit this new aspect fully, we will look for a solution to the equation of motion that oscillates with the same angular frequency, $\omega_{d}$, as the driving force.

We can relate [2.14](#eq-2-14) to an equation of motion with a complex driving force 
$$
\frac{d^{2}}{d t^{2}} z(t)+\Gamma \frac{d}{d t} z(t)+\omega_{0}^{2} z(t)=\mathcal{F}(t) / m , \tag{2.16} \label{eq-2-16}
$$

where 
$$
\mathcal{F}(t)=F_{0} e^{-i \omega_{d} t} . \tag{2.17} \label{eq-2-17}
$$

This works because the equation of motion, [2.14](#eq-2-14), does not involve $i$ explicitly and because 
$$
...
or 
$$
\mathcal{A}=\frac{F_{0} / m}{\omega_{0}^{2}-i \Gamma \omega_{d}-\omega_{d}^{2}} . \tag{2.21} \label{eq-2-21}
$$
...
$$
A=\frac{\left(\omega_{0}^{2}-\omega_{d}^{2}\right) F_{0} / T n}{\left(\omega_{0}^{2}-\omega_{d}^{2}\right)^{2}+\Gamma^{2} \omega_{d}^{2}} ; \tag{2.23} \label{eq-2-23}
$$

$$
B=\frac{\Gamma \omega_{d} F_{0} / m}{\left(\omega_{0}^{2}-\omega_{d}^{2}\right)^{2}+\Gamma^{2} \omega_{d}^{2}} \tag{2.24} \label{eq-2-24}
$$

Then the solution to the equation of motion for the real driving force, [2.14](#eq-2-14), is 
$$
x(t)=\operatorname{Re} z(t)=\operatorname{Re}\left(\mathcal{A} e^{-i \omega_{d} t}\right)=A \cos \omega_{d} t+B \sin \omega_{d} t . \tag{2.25} \label{eq-2-25}
$$

Thus the solution for the real force is a sum of two terms. The term proportional to $A$ is in phase with the driving force (or $180^{\circ}$ out of phase), while the term proportional to $B$ is $90^{\circ}$ out of phase. The advantage of going to the complex driving force is that it allows us to get both at once. The coefficients, $A$ and $B$, are shown in the graph in [Figure 2.4](#fig-2-4) for $\Gamma=\omega_{0} / 2$.

:::{figure} ../images/lt-34041-clipboard_ec1708d97d6bbe09eff1062f4dd4293a6.png
:label: fig-2-4
:enumerator: 2.4
:alt: The elastic and absorptive amplitudes, plotted versus \omega_{d}. The absorptive amplitude is the dotted line.
...
It is instructive to consider the work done by the external force in [2.16](#eq-2-16). **To do this we must use the real force, [2.14](#eq-2-14), and the real displacement [2.25](#eq-2-25), rather than their complex extensions, because, unlike almost everything else we talk about, the work is a nonlinear function of the force.** The power expended by the force is the product of the driving force and the velocity, 
$$
P(t)=F(t) \frac{\partial}{\partial t} x(t)=-F_{0} \omega_{d} A \cos \omega_{d} t \sin \omega_{d} t+F_{0} \omega_{d} B \cos ^{2} \omega_{d} t . \tag{2.26} \label{eq-2-26}
$$

The first term in [2.26](#eq-2-26) is proportional to $\sin 2 \omega_{d} t$. Thus it is sometimes positive and sometimes negative. It averages to zero over any complete half-period of oscillation, a time $\pi / \omega_{d}$, because 
$$
\int_{t_{0}}^{l_{0}+\pi / \omega_{d}} d t \sin 2 \omega_{d} t=-\left.\frac{1}{2} \cos 2 \omega_{d} t\right|_{t_{0}} ^{t_{0}+\pi / \omega_{d}}=0 . \tag{2.27} \label{eq-2-27}
$$
...
The second term in [2.26](#eq-2-26), on the other hand, is always positive. It averages to 
$$
P_{\text {average }}=\frac{1}{2} F_{0} \omega_{d} B . \tag{2.28} \label{eq-2-28}
$$

This is why $B$ is called the absorptive amplitude. It measures how fast energy is absorbed by the system. The absorbed power, $P_{\text{average}}$, reaches a maximum on resonance, at $\omega_{0} = \omega_{d}$. This is a diagnostic that is often used to find resonances in experimental situations. Note that the dependence of $B$ on $\omega_{d}$ looks qualitatively similar to that of $P_{\text{average}}$, which is shown in [Figure 2.5](#fig-2-5) for $\Gamma = \omega_{0} / 2$. However, they differ by a factor of $\omega_{d}$. In particular, the maximum of $B$ occurs slightly below resonance.

:::{figure} ../images/lt-34042-clipboard_e8ca031846509c308364060eef18b0ac5.png
:label: fig-2-5
:enumerator: 2.5
:alt: The average power lost to the frictional force as a function of \omega_{d} for \Gamma=\omega_{0} / 2.
...
Both the height and the width of the resonance curve in [Figure 2.5](#fig-2-5) are determined by the frictional term, $\Gamma$, in the equation of motion. The maximum average power is inversely proportional to $\Gamma$, 
$$
\frac{F_{0}^{2}}{2 m \Gamma} . \tag{2.29} \label{eq-2-29}
$$
...
$$
\omega_{1 / 2}=\sqrt{\omega_{0}^{2}+\frac{\Gamma^{2}}{4}} \pm \frac{\Gamma}{2} . \tag{2.30} \label{eq-2-30}
$$

The $\Gamma$ is the “full width at half-maximum” of the power curve. In [Figure 2.6](#fig-2-6) and [Figure 2.7](#fig-2-7), we show the average power as a function of $\omega_{d}$ for $\Gamma = \omega_{0} / 4$ and $\Gamma = \omega_{0}$. The linear dependence of the width on $\Gamma$ is clearly visible. The dotted lines show the position of half-maximum.

:::{figure} ../images/lt-34043-clipboard_e74d46b66dc2928fe81cf61b46e91524f.png
:label: fig-2-6
:enumerator: 2.6
:alt: The average power lost to the frictional force as a function of \omega_{d} for \Gamma = \omega_{0} / 4.
...
:label: fig-2-7
:enumerator: 2.7
:alt: The average power lost to the frictional force as a function of \omega_{d} for \Gamma = \omega_{0}.
...
$$
E=\hbar \omega \tag{2.31} \label{eq-2-31}
$$

where $\hbar$ is Planck’s constant divided by $2 \pi$, 
$$
h \approx 6.626 \times 10^{-34} \mathrm{Js} . \tag{2.32} \label{eq-2-32}
$$
...
We can also write [2.25](#eq-2-25) as 
$$
x(t)=R \cos \left(\omega_{d} t-\theta\right) \tag{2.33} \label{eq-2-33}
$$
for
$$
R=\sqrt{A^{2}+B^{2}}, \quad \theta=\arg (A+i B) . \tag{2.34} \label{eq-2-34}
$$
...
The phase lag goes through $\pi / 2$ at resonance, as shown in the graph in [Figure 2.8](#fig-2-8) for $\Gamma = \omega_{0} / 2$. A phase lag of $\pi / 2$ is another frequently used diagnostic for resonance.

:::{figure} ../images/lt-34045-clipboard_e4926805a22edf1e76f3bbd13d3c3c0a2.png
:label: fig-2-8
:enumerator: 2.8
:alt: A plot of the phase lag versus frequency in a damped forced oscillator.
...
:label: fig-2-9
:enumerator: 2.9
:alt: An oscillator that is damped by moving in a viscous fluid.
...
Dividing by $m$ and rearranging terms, you can see that this is identical to [2.14](#eq-2-14) with 
$$
F_{0} / m=K d_{0} / m=\omega_{0}^{2} d_{0} . \tag{2.38} \label{eq-2-38}
$$
...
The first step is to go over to the complex force, as in [2.16](#eq-2-16). The result looks like

![Figure](../images/lt-34049-clipboard_e4c4b4e55fdad338e4056d1e4e00be6c9.png)

$$
\frac{d^{2}}{d t^{2}} z(t)+\Gamma \frac{d}{d t} z(t)+\omega_{0}^{2} z(t)=\omega_{0}^{2} d_{0} e^{-i \omega_{d} t} \tag{2.39} \label{eq-2-39}
$$

We have labeled the terms in [2.39](#eq-2-39) to remind you of their different physical origins.

The next step is to look for irreducible steady state solutions of the form of [2.19](#eq-2-19): 
$$
z(t)=\mathcal{A} e^{-i \omega_{d} t} . \tag{2.40} \label{eq-2-40}
$$

Inserting [2.40](#eq-2-40) into [2.39](#eq-2-39), we get 
$$
\left[-\omega_{d}^{2}-i \Gamma \omega_{d}+\omega_{0}^{2}\right] \mathcal{A} e^{-i \omega_{d} t}=\omega_{0}^{2} d_{0} e^{-i \omega_{d} t} . \tag{2.41} \label{eq-2-41}
$$

What we will discuss in detail is the phase of the quantity in square brackets on the left-hand side of [2.41](#eq-2-41). Each of the three terms, inertial, frictional and spring, has a different phase. Each term also depends on the angular frequency, $\omega_{d}$ in a different way. The phase of $\mathcal{A}$ depends on which term dominates.

For very small $\omega_{d}$, in particular for 
$$
\omega_{d} \ll \omega_{0}, \Gamma , \tag{2.42} \label{eq-2-42}
$$
...
$$
\omega_{d} \gg \omega_{0}, \Gamma, \tag{2.43} \label{eq-2-43}
$$
...
$$
\mathcal{A} \approx-\frac{\omega_{0}^{2}}{\omega_{d}^{2}} d_{0} . \tag{2.44} \label{eq-2-44}
$$
