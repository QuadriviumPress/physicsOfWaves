...
$$
\omega^{2}=\frac{K_{a} a}{\rho_{L}} k^{2} \tag{7.3} \label{eq-7-3}
$$

where we have used the Taylor series expansion of $\sin x$, [1.58](#eq-1-58), and kept only the first term.

:::{figure} ../images/lt-33562-clipboard_e6e01f54a3b2b1604e7b8d44da02ba954.png
:label: fig-7-1
:enumerator: 7.1
:alt: Half a spring has twice the spring constant.

Half a spring has twice the spring constant.
:::
According to the discussion above, we can rewrite this as 
$$
\omega^{2}=\frac{K \ell}{\rho_{L}} k^{2} \tag{7.4} \label{eq-7-4}
$$
...
$$
\psi(0, t)=0, \quad \psi(\ell, t)=0 . \tag{7.6} \label{eq-7-6}
$$

The modes of the system are the same as for any other space translation invariant system. The linear combinations of the complex exponential modes of the infinite system that satisfy [7.6](#eq-7-6) are 
$$
A_{n}(x)=\sin \frac{n \pi x}{\ell} , \tag{7.7} \label{eq-7-7}
$$

with angular wave number 
$$
k_{n}=\frac{n \pi}{\ell} \tag{7.8} \label{eq-7-8}
$$

and frequency (from the dispersion relation, [7.4](#eq-7-4)) 
$$
\omega_{n}=\sqrt{\frac{K \ell}{\rho_{L}}} k_{n}=\sqrt{\frac{K \ell}{\rho_{L}}} \frac{n \pi}{\ell} . \tag{7.9} \label{eq-7-9}
$$

However, because the oscillations are longitudinal, the modes **look** very different from the transverse modes of the string that we studied in the previous chapter. The position of the point on the string whose equilibrium position is x, in the nth normal mode, has the general form (from [7.5](#eq-7-5)) 
$$
x+\epsilon \sin \frac{n \pi x}{\ell} \cos \left(\omega_{n} t+\phi\right) \tag{7.10} \label{eq-7-10}
$$

where $\epsilon$ and $\phi$ are the amplitude and phase of the oscillation.

The lowest 9 modes in [7.10](#eq-7-10) are animated in program 7-1. Compare these with the modes animated in program 6-1. The mathematics is the same, but the physics is very different because of [7.5](#eq-7-5). Stare at these two animations until you can visualize the relation between the two. Then you will have understood [7.5](#eq-7-5).

### Free Ends

Now let us look at the situation in which the end of the spring at $x = 0$ is fixed, but the end at $x = \ell$ is free. The boundary conditions in this case are analogous to the normal modes of the string with one fixed end. The displacement at $x = 0$ must vanish because the end is fixed. Also, the derivative of the displacement at $x = \ell$ must vanish. You can see this by looking at the continuous spring as the limit of discrete masses coupled by springs. As we saw in [5.43](#eq-5-43), the last real mass must have the same displacement as the first “imaginary” mass, 
$$
\psi(\ell, t)=\psi(\ell+a, t) . \tag{7.11} \label{eq-7-11}
$$

Therefore, for the finite system with a free end at $\ell$, we have the relation 
$$
\frac{\psi(\ell, t)-\psi(\ell+a, t)}{a}=0 \text { for all } a . \tag{7.12} \label{eq-7-12}
$$
...
$$
\left.\frac{\partial}{\partial x} \psi(x, t)\right|_{x=\ell}=0 . \tag{7.13} \label{eq-7-13}
$$

Thus the boundary conditions on the displacement are the same as in [6.11](#eq-6-11) for the transverse oscillation of a continuous string with $x = 0$ fixed and $x = \ell$ free, 
$$
\psi(0, t)=0,\left.\quad \frac{\partial}{\partial x} \psi(x, t)\right|_{x=\ell}=0 . \tag{7.14} \label{eq-7-14}
$$

This, in turn, implies that the normal modes are the same as for the transversely oscillating string, [6.15](#eq-6-15), 
$$
A_{n}(x)=\sin \left(\frac{(2 n+1) \pi x}{2 \ell}\right) \quad \text { for } n=0 \text { to } \infty . \tag{7.15} \label{eq-7-15}
$$

However, again because of [7.5](#eq-7-5), these modes look very different from those of the string. The first nine are animated in program 7-2 (compare with program 6-2).
...
with frequency 
$$
\omega_{n}=\sqrt{\frac{K \ell}{\rho_{L}}} k_{n} . \tag{7.17} \label{eq-7-17}
$$

As always, $k_{n}$ and $\omega_{n}$ are related by the dispersion relation, [7.4](#eq-7-4). Now to determine the possible values of $k_{n}$, we require that $F = ma$ be satisfied for the mass. Suppose, for example, that the amplitude of the oscillation is $A$ (a length). Then the displacement of the point on the spring with equilibrium position $x$ is 
$$
\psi(x, t)=A \sin k_{n} x \cos \omega_{n} t, \tag{7.18} \label{eq-7-18}
$$

and the displacement of the mass is determined by the displacement of the end of the spring, 
$$
x(t) \equiv \psi(\ell, t)=A \sin k_{n} \ell \cos \omega_{n} t . \tag{7.19} \label{eq-7-19}
$$

The acceleration is 
$$
a(t)=\frac{\partial^{2}}{\partial t^{2}} \psi(\ell, t)=-\omega_{n}^{2} A \sin k_{n} \ell \cos \omega_{n} t \tag{7.20} \label{eq-7-20}
$$

:::{figure} ../images/lt-33565-clipboard_e1fa8d9b58934f9c80ab87c8e878df571.png
:label: fig-7-2
:enumerator: 7.2
:alt: The stretching of the last spring is \psi(\ell, t)-\psi(\ell-a, t)

The stretching of the last spring is $\psi(\ell, t)-\psi(\ell-a, t)$
:::
To find the force on the mass, consider the massive spring as the continuum limit as $a \rightarrow 0$ of masses connected by massless springs of equilibrium length $a$, as at the beginning of the chapter. Then the force on the mass at the end is determined by the stretching of the last spring in the series. This, in turn, is the difference between the displacement of the system at $x = \ell$ and $x = \ell - a$, as illustrated in [Figure 7.2](#fig-7-2). Thus the force is 
$$
F=-K_{\dot{a}}[\psi(\ell, t)-\psi(\ell-a, t)] . \tag{7.21} \label{eq-7-21}
$$

In order to take the limit, $a \rightarrow 0$, rewrite this as 
$$
F=-K_{a} a \frac{\psi(\ell, t)-\psi(\ell-a, t)}{a} . \tag{7.22} \label{eq-7-22}
$$
...
$$
F=-\left.K \ell \frac{\partial}{\partial x} \psi(x, t)\right|_{x=\ell}=-K \ell k_{n} A \cos k_{n} \ell \cos \omega_{n} t . \tag{7.23} \label{eq-7-23}
$$

Note that the units work. $K \ell$ is a force. $\frac{\partial}{\partial x} \psi$ is dimensionless.

Putting [7.20](#eq-7-20) and [7.23](#eq-7-23) into $F = ma$ and canceling a factor of $-A \cos \omega_{n} t$ on both sides gives, 
$$
K \ell k_{n} \cos k_{n} \ell=m \omega_{n}^{2} \sin k_{n} \ell . \tag{7.24} \label{eq-7-24}
$$

Using the dispersion relation to eliminate $\omega_{n}^{2}$, we obtain 
$$
k_{n} \ell \tan k_{n} \ell=\frac{\rho_{L} \ell}{m} . \tag{7.25} \label{eq-7-25}
$$

We have multiplied both sides of [7.25](#eq-7-25) by $\ell$ in order to deal with the dimensionless variables $k_{n}\ell$ (which is $2 \pi$ times the number of wavelengths that fit onto the spring) and the dimensionless number 
$$
\epsilon \equiv \frac{\rho_{L} \ell}{m} \tag{7.26} \label{eq-7-26}
$$
...
The important point is that [7.25](#eq-7-25) has only one solution for $k_{n}\ell$ that goes to zero as $\epsilon \rightarrow 0$. Because $\tan k \ell \approx k \ell$ for small $k \ell$, it is 
$$
k_{0} \ell \approx \sqrt{\epsilon} \text { . } \tag{7.27} \label{eq-7-27}
$$

For all the other solutions, the smallness of the left-hand side of [7.25](#eq-7-25) must come because $\tan k_{n} \ell$ is very small, 
$$
k_{n} \ell \approx n \pi \quad \text { for } n=1 \text { to } \infty . \tag{7.28} \label{eq-7-28}
$$

But [7.28](#eq-7-28) implies 
$$
x(t) \equiv \psi(\ell, t)=A \sin k_{n} \ell \cos \omega_{n} t \approx 0 \quad \text { for } n=1 \text { to } \infty . \tag{7.29} \label{eq-7-29}
$$
...
$$
\omega_{n} \approx n \pi \sqrt{\frac{K}{\rho_{L} \ell}} \quad \text { for } n=1 \text { to } \infty , \tag{7.30} \label{eq-7-30}
$$

while the frequency of the $k_{0}$ mode is 
$$
\omega_{0} \approx \sqrt{\frac{K}{m}} . \tag{7.31} \label{eq-7-31}
$$
...
<sup>2</sup>Note that we can use this to give an alternate derivation of the boundary condition for a free end, [7.14](#eq-7-14).
...
$$
\rho_{L}=\rho A \tag{7.32} \label{eq-7-32}
$$
...
:label: fig-7-4
:enumerator: 7.4
:alt: The organ pipe with a piston at the top. The air in the tube acts like a spring.
...
:label: fig-7-5
:enumerator: 7.5
:alt: Pushing in the piston changes the volume of the air in the tube.
...
$$
p \propto V^{-\gamma} \tag{7.34} \label{eq-7-34}
$$
...
$$
C_{P} / C_{V} \tag{7.35} \label{eq-7-35}
$$

In air, at standard temperature and pressure 
$$
\gamma_{\text {air }} \approx 1.40 \tag{7.36} \label{eq-7-36}
$$

Now we can write from [7.34](#eq-7-34), 
$$
\frac{d p}{p}=-\gamma \frac{d V}{V} \tag{7.37} \label{eq-7-37}
$$

or 
$$
d p=-\gamma p \frac{d V}{V} \approx \frac{\gamma A p_{0}}{V} d z=\frac{\gamma p_{0}}{\ell} d z \tag{7.38} \label{eq-7-38}
$$

where $p_{0}$ is the equilibrium (room) pressure. Then the force on the piston is 
$$
d F=A d p=\frac{\gamma A^{2} p_{0}}{V} d z=\frac{\gamma A p_{0}}{\ell} d z \tag{7.39} \label{eq-7-39}
$$

so that 
$$
K=\frac{d F}{d z}=\frac{\gamma A p_{0}}{\ell} \tag{7.40} \label{eq-7-40}
$$

and $K \ell$ is 
$$
K \ell=\gamma A p_{0} . \tag{7.41} \label{eq-7-41}
$$

Thus we expect the dispersion relation to be 
$$
\omega^{2}=v_{\text {sound }}^{2} k^{2}=\frac{K \ell}{\rho_{L}} k^{2}=\frac{\gamma p_{0}}{\rho} k^{2} \tag{7.42} \label{eq-7-42}
$$

where we have defined the “speed of sound”, $v_{\text {sound }}$, as 
$$
v_{\text {sound }}^{2}=\frac{\gamma p_{0}}{\rho} \tag{7.43} \label{eq-7-43}
$$

For air at standard temperature and pressure, 
$$
v_{\text {sound }} \approx 332 \frac{\mathrm{m}}{\mathrm{s}} . \tag{7.44} \label{eq-7-44}
$$
...
$$
p-p_{0}=-\gamma p_{0} \frac{\partial}{\partial z} \psi . \tag{7.46} \label{eq-7-46}
$$

We want the negative sign because for $\frac{\partial}{\partial z} \psi>0$, the air is spreading out and has lower pressure.

Thus for a standing wave in the pipe, [7.3](#eq-7-3), we expect the boundary conditions 
$$
\psi(0, t)=0,\left.\quad \frac{\partial}{\partial z} \psi(z, t)\right|_{z=\ell}=0 , \tag{7.47} \label{eq-7-47}
$$

for which the solution is 
$$
\psi(z, t)=\sin k z \cos \omega t \tag{7.48} \label{eq-7-48}
$$

$$
k=\frac{(n+1 / 2) \pi}{\ell}, \quad \omega=v k , \tag{7.49} \label{eq-7-49}
$$

where $v=v_{\text {sound }}$, for nonnegative integer $n$. In particular, the lowest frequency mode of the tube corresponds to $n = 0$, 
$$
\omega=\frac{v \pi}{2 \ell}, \quad \nu=\frac{\omega}{2 \pi}=\frac{v}{4 \ell} . \tag{7.50} \label{eq-7-50}
$$

### Helmholtz Approximation

Let’s consider a slightly different problem. What is the lowest frequency mode of a one-liter soda bottle, shown in [Figure 7.6](#fig-7-6)? A typical set of parameters is given below: 
$$
\begin{aligned}
...
and to treat the body as a spring, that contributes restoring force but no inertia (because the air is not moving much). Then all we must do is to compute the $K$ of the “spring.” That is easy, using [7.38](#eq-7-38). In this case, 
$$
d V=A d z , \tag{7.53} \label{eq-7-53}
$$

so 
$$
d p=-\gamma p \frac{A d z}{V} \approx-\gamma p_{0} \frac{A d z}{V_{0}} \tag{7.54} \label{eq-7-54}
$$
...
and 
$$
F \approx-\gamma p_{0} \frac{A^{2} d z}{V_{0}} \tag{7.55} \label{eq-7-55}
$$

or 
$$
" K "=\gamma p_{0} \frac{A^{2}}{V_{0}} . \tag{7.56} \label{eq-7-56}
$$

Then using $\omega^{2}=K / m$, we expect 
$$
\omega=\sqrt{\frac{\gamma A^{2} p_{0} / V_{0}}{\rho A \ell}}=v \sqrt{\frac{A}{\ell V_{0}}} . \tag{7.57} \label{eq-7-57}
$$

For the soda bottle, [7.6](#eq-7-6), this gives 
$$
...
$$
y \cos \frac{\omega z}{v} \tag{7.60} \label{eq-7-60}
$$
...
$$
y \cos \frac{\omega \ell}{v} . \tag{7.61} \label{eq-7-61}
$$

The force at this point from the compression of the air in the neck is (from [7.45](#eq-7-45)) 
$$
F_{\text {neck }}=-\gamma A p_{0} \frac{\partial \psi}{\partial z}=\frac{\gamma A p_{0} \omega}{v} y \sin \frac{\omega \ell}{v} . \tag{7.62} \label{eq-7-62}
$$

This must be the negative of the force from the air in the body, from [7.39](#eq-7-39), 
$$
-F_{\text {body }}=\frac{\gamma A^{2} p_{0}}{V_{0}} y \cos \omega \ell / v , \tag{7.63} \label{eq-7-63}
$$
